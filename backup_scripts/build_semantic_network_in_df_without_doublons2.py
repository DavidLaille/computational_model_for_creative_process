...
for i, row in df.iterrows():
    ...
    for d in range(depth):
        ...
        for w in range(len(current_words)):
            words_in_memory = []
            for index in range(d+1):  # on rajoute 1 pour éviter le décalage (car le décours commence à 0)
                print("index_ligne : ", index_row, "index_colonne : ", index)
                words_in_memory.append(matrix.iloc[index_row, index])
            print(words_in_memory)
            ...
            # si on veut prendre l'ensemble des mots rencontrés jusqu'ici
            # neighbours_current_word = model.most_similar(visited_words, topn=N, restrict_vocab=10000)
            # si on veut prendre en compte uniquement la chaîne de mots précédents
            neighbours_current_word = model.most_similar(words_in_memory, topn=N, restrict_vocab=10000)
