...
discounting_rates = np.arange(start=0.01, stop=0.50, step=0.01)
...
for cue in df['cues']:
    ...
    for discounting_rate in discounting_rates:
        ...
        # Calcul des moyennes pour les valeurs numériques
        ...
        # print(paths)
        paths_filename = f'dataframes/test_discounting_rate_{cue}_paths_{round(discounting_rate, 3)}.csv'
        paths.to_csv(paths_filename, index=False, sep=',')

        # print(all_neighbours_data)
        all_neighbours_data_filename = f'dataframes/test_discounting_rate_{cue}_all_neighbours_data_{round(discounting_rate, 3)}.csv'
...
########################################################################################################################
    ...
    file_name = f"images/test_discounting_rate_{cue}_Li_vs_Nb_steps.png"
    print(file_name)
    plt.savefig(file_name)
    # plt.show()
########################################################################################################################
...
########################################################################################################################
    ...
    fig_graphs.suptitle(f'{cue} - Influence de discounting_rate sur les sorties du modèle',
                        color='brown', fontsize=14)
    ...
    axs[1, 0].scatter(discounting_rates_extended_axis, nb_steps, color='blue', marker='o')
    ...
    axs[0, 1].set(xlabel='discounting_rate', ylabel='similarity_chosen_word_means')
    axs[1, 1].scatter(discounting_rates_extended_axis, similarity_chosen_word, color='orange', marker='o')
    ...
    axs[3, 0].scatter(discounting_rates_extended_axis, likeability_chosen_word, color='green', marker='o')
    ...
    axs[3, 1].scatter(discounting_rates_extended_axis, final_goal_value, color='red', marker='o')
...
########################################################################################################################
    ...
    # Number of steps
    axs[0, 0].plot(discounting_rates, nb_steps_max_q_value, color='blue')
    axs[0, 0].set_title('Nb_steps X discounting_rates')
    axs[0, 0].set(xlabel='discounting_rate', ylabel='nb_steps_max_q_value')

    # Similarity between the cue and the chosen word
    axs[0, 1].plot(discounting_rates, similarity_chosen_word_max_q_value, color='orange')
    axs[0, 1].set_title('similarity_chosen_word X discounting_rates')
    axs[0, 1].set(xlabel='discounting_rate', ylabel='similarity_chosen_word_max_q_value')

    # Likeability between the cue and the chosen word
    axs[1, 0].plot(discounting_rates, likeability_chosen_word_max_q_value, color='green')
    ...
    axs[1, 1].plot(discounting_rates, final_goal_value_max_q_value, color='red')
    ...
    # Sauvegarde des figures obtenues
    file_name = f"images/test_discounting_rate_{cue}_graphs2.png"
    print(file_name)
    plt.savefig(file_name)
