...
for cue in df['cues']:
    ...
    for method in methods:
        ...
        for t in range(nb_try):
            ...
            methods_extended_axis.append(method)
        ...
        # print(paths)
        paths_filename = f'dataframes/test_method_{cue}_paths_{method}.csv'
        paths.to_csv(paths_filename, index=False, sep=',')

        # print(all_neighbours_data)
        all_neighbours_data_filename = f'dataframes/test_method_{cue}_all_neighbours_data_{method}.csv'
...
########################################################################################################################
    ...
    file_name = f"images/test_method_{cue}_first_and_chosen_words.png"
    print(file_name)
...
########################################################################################################################
    ...
    file_name = f"images/test_method_{cue}_Li_vs_Nb_steps.png"
    print(file_name)
    plt.savefig(file_name)
    # plt.show()
########################################################################################################################
...
########################################################################################################################
    ...
    fig_graphs.suptitle(f'{cue} - Influence de method sur les sorties du modèle',
                        color='brown', fontsize=14)
    fig_graphs.tight_layout(h_pad=4, w_pad=4)
    plt.subplots_adjust(top=0.85, bottom=0.1, left=0.1, right=0.9)

    axs[0, 0].plot(methods, nb_steps_means, color='blue')
    axs[0, 0].set_title('Nb_steps X method')
    axs[0, 0].set(xlabel='method', ylabel='nb_steps_means')
    axs[1, 0].plot(methods_extended_axis, nb_steps, color='blue', marker='+')
    axs[1, 0].set_title('Nb_steps X method')
    axs[1, 0].set(xlabel='methods', ylabel='nb_steps')

    axs[0, 1].plot(methods, similarity_chosen_word_means, color='orange')
    axs[0, 1].set_title('similarity_chosen_word X methods')
    axs[0, 1].set(xlabel='method', ylabel='similarity_chosen_word')
    axs[1, 1].plot(methods_extended_axis, similarity_chosen_word, color='orange', marker='+')
    axs[1, 1].set_title('similarity_chosen_word X method')
    axs[1, 1].set(xlabel='methods', ylabel='similarity_chosen_word')

    axs[2, 0].plot(methods, likeability_chosen_word_means, color='green')
    axs[2, 0].set_title('likeability_chosen_word X method')
    axs[2, 0].set(xlabel='method', ylabel='likeability_chosen_word_means')
    axs[3, 0].plot(methods_extended_axis, likeability_chosen_word, color='green', marker='+')
    axs[3, 0].set_title('likeability_chosen_word X method')
    axs[3, 0].set(xlabel='methods', ylabel='likeability_chosen_word')

    axs[2, 1].plot(methods, final_goal_value_means, color='red')
    axs[2, 1].set_title('final_goal_value X method')
    axs[2, 1].set(xlabel='method', ylabel='final_goal_value_means')
    axs[3, 1].plot(methods_extended_axis, final_goal_value, color='red', marker='+')
    axs[3, 1].set_title('final_goal_value X method')
    axs[3, 1].set(xlabel='methods', ylabel='final_goal_value')

    # Sauvegarde des figures obtenues
    file_name = f"images/test_method_{cue}_graphs.png"
    print(file_name)
    plt.savefig(file_name)
########################################################################################################################
...
########################################################################################################################
    ...
    # Number of steps
    axs[0, 0].plot(methods, nb_steps_max_q_value, color='blue')
    ...
    axs[0, 1].plot(methods, similarity_chosen_word_max_q_value, color='orange')
    ...
    axs[1, 0].plot(methods, likeability_chosen_word_max_q_value, color='green')
    ...
    axs[1, 1].plot(methods, final_goal_value_max_q_value, color='red')
    ...
    # Sauvegarde des figures obtenues
    file_name = f"images/test_method_{cue}_graphs2.png"
    print(file_name)
    plt.savefig(file_name)
