...
for cue in df['cues']:
    ...
    for method in methods:
        ...
        # print(paths)
        paths_filename = f'dataframes/test_method_{cue}_paths_{discounting_rate}.csv'
        paths.to_csv(paths_filename, index=False, sep=',')

        # print(all_neighbours_data)
        all_neighbours_data_filename = f'dataframes/test_method_{cue}_all_neighbours_data_{discounting_rate}.csv'
    ...
    fig.suptitle(f'{cue} - Influence de method sur les sorties du modèle',
                 color='brown', fontsize=14)
